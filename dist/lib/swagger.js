...
(function () {
  ...
  SwaggerOperation.prototype.urlify = function (args) {
    ...
    // Detects if method is a request to atomic node
    var isAtomicMethod = function(nickname) {
      return [
        'getAtomicMethod',
        'postAtomicMethod',
        'putAtomicMethod',
    ...
    for (var i = 0; i < params.length; i++) {
      ...
      if (param.paramType === 'path') {
        ...
        if (typeof val !== 'undefined') {
          ...
          if (!param.unencoded) {
            // Do not encode URI if fusion node proxy endpoint requested
            if (!isAtomicMethod(this.nickname)) {
